...
En este proyecto, nos propusimos analizar el uso de las Ecobici en el año 2022, y la manera en que fluctua su uso dependiendo de distintas características como clima, horarios, día de la semana, genero de los usuarios, tipo de bici, eventos sociales, entre otros. 

Durante el año 2022, se registraron un total de 3.000.000 de recorridos de los cuales para este analisis, unicamente hemos analizado una muestra extraída del dataset original, que contiene 10.000 recorridos. En consecuencia, los resultados de nuestro análisis van a representar a esta muestra, pero no necesariamente el conjunto total.
...
- Dataset *MeteoStat-Aeroparque*: Esta base de datos corresponde a observaciones climáticas recolectadas por la estación meteorológica de Aeroparque desde el primero de enero de 2022 hasta el 31 de diciembre de 2022.
Como los dataframes utilizados se encuentran en formato `.csv`, utilizamos la librería `readr`.
...
Como ejemplo antes de comenzar con el análisis descriptivo, vamos a proceder a mostrarles una visualización del dataframe, con el que vamos a trabajar, analizar y elaborar hipótesis.

Este dataframes incluye otras variables extras a diferencia del dataframe original, lo cual nos permitio generar mas preguntas que podríamos hacer durante el análisis del proyecto.
...
Una pregunta interesante es ¿Cuanto suelen durar los viajes de los usuarios?, ¿Cual es la cantidad? y ¿Cuanto suelen durar los viajes de los usuarios según genero?
Esta pregunta podría llegar a responder cuestiones como que distancia debería haber entre estaciones y cual es la resistencia que tienen los usuarios. Pues, por ejemplo, si las estaciones estuviesen mas distanciadas entre si, esto implicaría que los usuarios deberían recorrer por mas tiempo, entonces los usuarios con menos resistencia, buscarian otros métodos de transporte.
...
#### Fecha de recorridos
En los siguientes gráficos, buscamos mostrar la cantidad de recorridos realizados durante distintos lapsos de tiempos tales como meses, días y horarios. 
Notamos que hay una tendencia al incremento de uso de bicis durante meses otoñales y primaverales (¿Sera debido al clima y temperaturas?) y días de semanas (¿Días de trabajo, clases?)
...
Otra pregunta que nos surgió durante el análisis fue con respecto al uso de bicis según genero ¿Hay algun genero que tienda a utilizar las bicis mas que otro?
Notamos que la proporción de hombres es mucha mayor.

```{r}
gr_use_of_bikes_by_gender 
```

Otra pregunta que nos interesaba analizar era el uso de bicicletas según genero y horario. Esto surge de la proposicion de que una persona de genero no masculino, quizás tienda a sentirse mas insegurx durante horarios nocturnos, en los cuales pueden ser mas vulnerables.
Para esta representación usamos cantidades porcentuales, en vez de cantidad de viajes porque al haber muchos usuarios masculinos, los gráficos iban a parecer sesgados.
...
Los eventos que seleccionamos fueron de forma arbitraria y fueron 8 días que están descritos en el informe.

Como vimos antes es mas común que haya mas viajes durante los días de semana que los fin de semana. Por ende vamos a analizar los eventos que ocurrieron durante estos días por separado
Notamos que dependiendo del día del evento en promedio
...
Este dataframe incluye otras variables extras a diferencia del dataframe original, lo cual nos permitio generar mas preguntas que podríamos hacer durante el análisis del proyecto.
...
Los siguientes gráficos representan el cambio de la temperatura máxima y mínima de cada dia del 2022
```{r}
...
Un tema de interés que nos surgió es el uso de bicis según temperatura promedio. ¿Sera que los dias mas calidos o frios tengan menos observaciones de usos de bicis?
Notamos que este gráfico en particular tiene un problema, las temperaturas promedias durante los 365 no se distribuyen de forma equitativa,por ejemplo es mucho mas común tener un día con promedio de 20 grados que un dia con promedio de 8 grados, entonces sumar el uso de bicis dependiendo de la temperatura promedio estaría sesgado.
```{r}
gr_bikes_by_temp
```
...
Otro pregunta que nos surgio durante el trabajo es ¿Sera que los días de lluvia tengan menos observaciones de usos de bicis?
Al principio de la hipotesis nosotros pensamos que habria una gran diferencia entre uso de bicis durante dias de lluvias vs dias de no lluvia, pero nos sorprendio ver que la diferencia fuese muy poca.
...
Separando de esta forma notamos que los dias de lluvias mas fuertes tienen un promedio mas bajo que el de dia de lluvias debiles. Lo cual responde en cierta parte a nuestra pregunta, de que lluvias fuertes reducen el uso de bicis.

```{r}
gr_potencia_lluvia_2022
```
Finalmente, tambien nos intereso observar la duracion de viajes segun dias lluviosos y no lluvios. ¿Sera que los recorridos durante dias de lluvia son mas cortos?
A partir del grafico observamos que la duracion se mantiene, lo cual refuta nuestra idea.
```{r}
